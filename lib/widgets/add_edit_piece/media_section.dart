...
class MediaSectionWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final regularItems = musicPiece.mediaItems
        .where((item) => item.type != MediaType.thumbnails)
        .toList();
    final thumbnailItems = musicPiece.mediaItems
        .where((item) => item.type == MediaType.thumbnails)
        .toList();
    ...
    void handleDelete(MediaItem deletedItem) {
      ...
      if (deletedItem.type == MediaType.thumbnails &&
          musicPiece.thumbnailPath == deletedItem.pathOrUrl) {
    ...
    Future<void> handleSetThumbnail(String thumbnailPath) async {
      if (thumbnailPath.isEmpty) {
        onMusicPieceChanged(
          musicPiece.copyWith(thumbnailPath: null, clearThumbnail: true),
        );
        return;
      }
      ...
      final isFromThumbnailWidget = musicPiece.mediaItems.any(
        (item) =>
            item.type == MediaType.thumbnails &&
            item.pathOrUrl == thumbnailPath,
      );

      if (isFromThumbnailWidget) {
        // Just set the path
        final widgetId = musicPiece.mediaItems
            .firstWhere(
              (item) =>
                  item.type == MediaType.thumbnails &&
                  item.pathOrUrl == thumbnailPath,
      ...
      try {
        ...
        final pieceMediaDir = Directory(
          p.join(appDir.path, 'media', musicPiece.id),
        );
        ...
        if (await sourceFile.exists()) {
          ...
          if (result != null) {
            ...
            AppLogger.log(
              'MediaSectionWidget: Compressed thumbnail to $compressedFilePath',
            );

            // Create or update thumbnail widget
            final updatedMediaItems = List<MediaItem>.from(
              musicPiece.mediaItems,
            );
            final existingThumbnailIndex = updatedMediaItems.indexWhere(
            ...
            if (existingThumbnailIndex != -1) {
              ...
              updatedMediaItems[existingThumbnailIndex] = oldItem.copyWith(
            ...
            AppLogger.log(
              'MediaSectionWidget: Compression failed for $thumbnailPath',
            );
          }
        } else {
          AppLogger.log(
            'MediaSectionWidget: Source thumbnail file not found: $thumbnailPath',
          );
        }
      } catch (e) {
        ...
        if (musicPiece.mediaItems.isEmpty)
          Center(
            child: Padding(
              ...
              child: Text(context.l10n.useThePlusSignToAddMedia),
            ),
          )
        else ...[
          if (regularItems.isNotEmpty)
            ReorderableListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final originalIndex = musicPiece.mediaItems.indexWhere(
                            ...
                            child: Column(
                              children: [
                                ...
                                IconButton(
                                  ...
                                  tooltip: context.l10n.deleteMediaItem,
                                ),
                            ...
                            child: Padding(
                              ...
                              child: MediaSection(
                                ...
                                onUpdateMediaItem: (updatedItem) {
                                  ...
                                  if (originalIndex != -1) {
                                    updatedMediaItems[originalIndex] =
                                        updatedItem;

                                    String? newPieceThumbnail =
                                        musicPiece.thumbnailPath;
                                    if (updatedItem.type ==
              ...
              onReorder: (oldIndex, newIndex) {
                ...
                onMusicPieceChanged(
                  musicPiece.copyWith(mediaItems: updatedMediaItems),
                );
              },
            ),
          ...
          if (thumbnailItems.isNotEmpty) ...[
            ...
            Padding(
              ...
              child: Text(
                context.l10n.thumbnailWidgetVisibleInEditModeOnly,
                style: TextStyle(
                  fontSize: 14,
            ...
            ...thumbnailItems.map((item) {
              ...
              return HighlightableWidget(
                ...
                child: MediaSection(
                  ...
                  isReorderable: false,
                  isTitleEditable:
                      false, // Disable title editing for thumbnail widget
                  isPathEditable:
                      false, // Disable path editing for thumbnail widget
